...
/**
 * E-01 单元测试 �?验证消息 JSON 字�?正确�?
 * 覆盖：ping, Login, register, startMatch, pve startMatch, Ready, move, Resign,
 * cancelMatch, requestFirstHand, drawRequest, drawResponse, queryGameRecords, queryGameRecord
 */
public class MessageBuilderTest {
    ...
    @Test
    void testStartMatchWithMode() throws Exception {
        String json = MessageBuilder.buildStartMatch("pve");
        JsonNode node = MAPPER.readTree(json);
        assertEquals("startMatch", node.get("messageType").asText());
        assertEquals("pve", node.get("mode").asText());
        assertEquals("human", node.get("clientType").asText());
    ...
    @Test
    void testQueryGameRecords() throws Exception {
        String json = MessageBuilder.buildQueryGameRecords(20, 10);
        JsonNode node = MAPPER.readTree(json);
        assertEquals("queryGameRecords", node.get("messageType").asText());
        assertEquals(20, node.get("offset").asInt());
        assertEquals(10, node.get("limit").asInt());
        assertEquals(3, node.size());
    }

    @Test
    void testQueryGameRecord() throws Exception {
        String json = MessageBuilder.buildQueryGameRecord("record-001");
        JsonNode node = MAPPER.readTree(json);
        assertEquals("queryGameRecord", node.get("messageType").asText());
        assertEquals("record-001", node.get("recordId").asText());
